...
import com.diazmain.obapp.Home.model.meals.MealTime
...
import kotlinx.android.synthetic.main.dialog_swap_fruits.*
import kotlinx.android.synthetic.main.dialog_swap_meats.*
import kotlinx.android.synthetic.main.dialog_swap_nuts.*
import kotlinx.android.synthetic.main.fragment_desayuno.*
import java.text.SimpleDateFormat
import java.util.*
import kotlin.collections.ArrayList

class DesayunoFragment : Fragment(), CompoundButton.OnCheckedChangeListener, View.OnClickListener, AdapterView.OnItemSelectedListener {

    lateinit var desMenu: OptionsPerMeal
    lateinit var cambiopor: ArrayList<String>
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        desMenu = (activity as ReminderActivity).mealsMenu.menuCal.desayuno

        val sp: Spinner = (activity as ReminderActivity).findViewById(R.id.spDes)
    ...
    /*private fun fillSwapsList() {
        clearSwaps()
        (activity as ReminderActivity).swaps.add(FoodSwapInfo(1, cbd1.text.toString(), ArrayList<String>()))
        (activity as ReminderActivity).swaps.add(FoodSwapInfo(1, cbd2.text.toString(), ArrayList<String>()))
        (activity as ReminderActivity).swaps.add(FoodSwapInfo(1, cbd3.text.toString(), ArrayList<String>()))
        (activity as ReminderActivity).swaps.add(FoodSwapInfo(1, cbd4.text.toString(), ArrayList<String>()))
        (activity as ReminderActivity).swaps.add(FoodSwapInfo(1, cbd5.text.toString(), ArrayList<String>()))
        (activity as ReminderActivity).swaps.add(FoodSwapInfo(1, cbd6.text.toString(), ArrayList<String>()))
    }

    private fun clearSwaps() {
    ...
    private fun getDialog1(i: Int, meal: ArrayList<IndividualMealArrayElement>, op: Int): AlertDialog {
        ...
        when(op) {
            0 -> { cambioUno = desMenu.op1[i].cambio.cambioUno }
            1 -> { cambioUno = desMenu.op2[i].cambio.cambioUno }
            2 -> { cambioUno = desMenu.op3[i].cambio.cambioUno }
            3 -> { cambioUno = desMenu.op4[i].cambio.cambioUno }
            4 -> { cambioUno = desMenu.op5[i].cambio.cambioUno }
        }
            ...
            for (x in swapOne.indices) {
                if (swapOne[x].isChecked) {
                    if ((activity as ReminderActivity).swaps.size == 0) {
                        (activity as ReminderActivity).swaps.add(FoodSwapInfo(1, i, meal[i].cbox.text.toString(), swapOne[x].text.toString(), "", ""))
                    } else {
                        val it = (activity as ReminderActivity).swaps.iterator()
                        while(it.hasNext()) {
                            val food: FoodSwapInfo = it.next()
                    ...
                    /*if (!meal[i].swap.contains(swapOne[x].text.toString()))
                        meal[i].swap.add(swapOne[x].text.toString())*/

                    //cambiopor.add(swapOne[x].text.toString())
                }
            }
            //Log.w("Swap 1 size", meal[i].swap.size.toString())
        })
    ...
    private fun getDialog2(i: Int, meal: ArrayList<IndividualMealArrayElement>, op: Int): AlertDialog {
        ...
        when(op) {
            0 -> { cambioDos = desMenu.op1[i].cambio.cambioDos }
            1 -> { cambioDos = desMenu.op2[i].cambio.cambioDos }
            2 -> { cambioDos = desMenu.op3[i].cambio.cambioDos }
            3 -> { cambioDos = desMenu.op4[i].cambio.cambioDos }
            4 -> { cambioDos = desMenu.op5[i].cambio.cambioDos }
        }
        ...
        alBuilder.setPositiveButton(R.string.btn_accept, DialogInterface.OnClickListener() { dialog, which ->
            ...
            for (x in swapTwo.indices) {
                if (swapTwo[x].isChecked) {
                    if ((activity as ReminderActivity).swaps.size == 0) {
                        (activity as ReminderActivity).swaps.add(FoodSwapInfo(1, i, meal[i].cbox.text.toString(), "", swapTwo[x].text.toString(), ""))
                    } else {
                        val it = (activity as ReminderActivity).swaps.iterator()
                        while(it.hasNext()) {
                            val food: FoodSwapInfo = it.next()
                    ...
                    /*if (!meal[i].swap.contains(swapTwo[x].text.toString()))
                        meal[i].swap.add(swapTwo[x].text.toString())*/
                    //cambiopor.add(swapTwo[x].text.toString())
                }
            }
            Log.w("Swap 2 size", meal[i].swap.size.toString())
        })
        alBuilder.setNegativeButton(R.string.btn_cancel, DialogInterface.OnClickListener() { dialog, which ->
            meal[i].swap.clear()
            dialog.cancel()
    ...
    private fun getDialog3(i: Int, meal: ArrayList<IndividualMealArrayElement>, op: Int): AlertDialog {
        ...
        when(op) {
            0 -> { cambioTres = desMenu.op1[i].cambio.cambioTres }
            1 -> { cambioTres = desMenu.op2[i].cambio.cambioTres }
            2 -> { cambioTres = desMenu.op3[i].cambio.cambioTres }
            3 -> { cambioTres = desMenu.op4[i].cambio.cambioTres }
            4 -> { cambioTres = desMenu.op5[i].cambio.cambioTres }
        }
        ...
        alBuilder.setPositiveButton(R.string.btn_accept, DialogInterface.OnClickListener() { dialog, which ->
            ...
            for (x in swapThree.indices) {
                if (swapThree[x].isChecked) {
                    if ((activity as ReminderActivity).swaps.size == 0) {
                        (activity as ReminderActivity).swaps.add(FoodSwapInfo(1, i, meal[i].cbox.text.toString(), "", "", swapThree[x].text.toString()))
                    } else {
                        val it = (activity as ReminderActivity).swaps.iterator()
                        while(it.hasNext()) {
                            val food: FoodSwapInfo = it.next()
